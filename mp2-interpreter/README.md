...
`BoolExp` for booleans, `FunExp` for functions, `BindExp` for let expressions,
`AppExp` for function applications, `IfExp` for if expressions, `ArithExp` for
...
data Exp = IntExp Int
         | BoolExp Bool
         | FunExp [String] Exp
         | BindExp [(String,Exp)] Exp
         | AppExp Exp [Exp]
         | IfExp Exp Exp Exp
         | ArithExp String Exp Exp
...
interpreter, `IfStmt` for conditional statements, `DefStmt` for procedure
...
Modify `eval` to handle `ArithExp` so that we can evaluate arithmetic
...
``` {.haskell}
*Main> eval (ArithExp "+" (IntExp 5) (IntExp 4)) H.empty
9
*Main> eval (ArithExp "-" (ArithExp "*" (IntExp 3) (IntExp 10)) (IntExp 7)) H.empty
23
*Main> eval (ArithExp "/" (IntExp 6) (IntExp 2)) H.empty
3
*Main> eval (ArithExp "/" (IntExp 6) (IntExp 0)) H.empty
exn: Division by 0
*Main> eval (ArithExp "+" (IntExp 6) (ArithExp "/" (IntExp 4) (IntExp 0))) H.empty
exn: Cannot lift
```
...
*Main> let fun1 = FunExp ["a", "b"] (ArithExp "+" (VarExp "a") (VarExp "b"))
*Main> eval fun1 H.empty
<["a","b"], ArithExp "+" (VarExp "a") (VarExp "b"), fromList []>
*Main> let fun2 = FunExp ["k"] (ArithExp "*" (VarExp "k") (VarExp "x"))
*Main> eval fun2 env
<["k"], ArithExp "*" (VarExp "k") (VarExp "x"), fromList [("x",3)]>
...
Welcome to your interpreter!
> print fn [x] 2 * x end;
<["x"], ArithExp "*" (IntExp 2) (VarExp "x"), fromList []>
...
Modify `eval` to handle `BindExp`.

``` {.haskell}
*Main> eval (BindExp [] (IntExp 5)) H.empty
5
*Main> eval (BindExp [("x", IntExp 5)] (VarExp "x")) H.empty
5
*Main> eval (BindExp [("x", ArithExp "+" (IntExp 5) (IntExp 4))] (ArithExp "*" (VarExp "x") (IntExp 2))) H.empty
18
```
...
Modify `exec` to handle `DefStmt`s and `CallStmt`s. Procedures allows us
