...
intOpExpUnitTests =
  [ ( "(+)"
    , assertEqual' ""
      ( eval (ArithExp "+" (IntExp 5) (IntExp 4)) H.empty )
      $ IntVal 9
    )
  , ( "(+ lift exception) "
    , assertEqual' ""
      ( eval (ArithExp "+"
              (IntExp 6)
              (ArithExp "/" (IntExp 4) (IntExp 0))) H.empty )
      $ ExnVal "Cannot lift"
    )
  , ( "(/)"
    , assertEqual' ""
      ( eval (ArithExp "-"
              (ArithExp "*" (IntExp 3) (IntExp 10))
              (IntExp 7)) H.empty )
      $ IntVal 23
    )
  , ( "(/ exception)"
    , assertEqual' ""
      ( eval (ArithExp "/" (IntExp 6) (IntExp 0)) H.empty )
...
appExpUnitTests =
  [ ( "Multiple parameters"
    , assertEqual' ""
      ( eval (AppExp (FunExp ["a","b"] (ArithExp "+" (VarExp "a") (VarExp "b")))
...
letExpUnitTests =
  [ ( "Multiple bindings"
    , assertEqual' ""
      ( eval (BindExp [("a",IntExp 4),("b",IntExp 5)]
               (ArithExp "+" (VarExp "a") (VarExp "b")))
        H.empty )
      $ IntVal 9
    )
  , ( "Empty binding"
    , assertEqual' ""
      ( eval (BindExp [] (VarExp "x")) H.empty )
      $ ExnVal "No match in env"
    )
  , ( "non-trivial binding expression"
    , assertEqual' ""
      ( eval (BindExp [("b", VarExp "k")]
...
procStmtUnitTests =
  [ ( "Vanilla"
    , assertEqual' ""
      ( exec (DefStmt "p" [] (PrintStmt (VarExp "x"))) H.empty H.empty )
      ( ""
      , H.fromList [("p", DefStmt "p" [] (PrintStmt (VarExp "x")))]
      , H.empty
      )
    )
  , ( "Environment preserved"
    , assertEqual' ""
      ( exec (DefStmt "p" [] (PrintStmt (VarExp "x"))) H.empty testenv2 )
      ( ""
      , H.fromList [("p", DefStmt "p" [] (PrintStmt (VarExp "x")))]
      , testenv2)
    )
...
procFibHelp =
  ...
  (IfStmt (CompOpExp "<=" (VarExp "n") (IntExp 0))
    ...
    (SeqStmt [ SetStmt "tmp" $ VarExp "acc2"
             , SetStmt "acc2" $ ArithExp "+" (VarExp "acc1") (VarExp "acc2")
             , SetStmt "acc1" $ VarExp "tmp"
             , CallStmt "fibHelper" [ArithExp "-" (VarExp "n") (IntExp 1)]
             ]))
procFibonacci =
  DefStmt "Fibonacci" ["n"]
  $ SeqStmt [ procFibHelp
...
callStmtUnitTests =
  ...
  , ( "Higher Order Procedure"
    , assertEqual' ""
      ( exec ( SeqStmt [ DefStmt "fog" ["f", "g", "x"]
                         (SetStmt "x" (AppExp (VarExp "f")
                                       [(AppExp (VarExp "g") [VarExp "x"])]))
                       , CallStmt "fog"
                         [ FunExp ["x"] (ArithExp "*" (VarExp "x") (IntExp 2))
                         , FunExp ["x"] (ArithExp "+" (VarExp "x") (IntExp 1))
                         , IntExp 6
                         ]
                       , PrintStmt (VarExp "x")
                       ]
      ...
      ( "14"
      , H.fromList [("fog", DefStmt "fog" ["f", "g", "x"]
                      (SetStmt "x" (AppExp (VarExp "f")
                                    [(AppExp (VarExp "g") [VarExp "x"])])))
                   ]
      , H.fromList [ ("x", IntVal 14)
                   , ("f", CloVal ["x"]
                       (ArithExp "*" (VarExp "x") (IntExp 2)) H.empty)
                   , ("g", CloVal ["x"]
                       (ArithExp "+" (VarExp "x") (IntExp 1)) H.empty)
                   ]
      )
    )
  ]
